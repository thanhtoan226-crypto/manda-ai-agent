...
An **Browse Agents** landing page where users select an AI agent, which redirecting **Workbench** where users interact with AI agents to generate data-rich reports and view reports 
A **Pulse** where users view reports generated by AI agents manually or scheduled . 
All AI responses and data are stored in mockdata folder.
...
| `/workbench` | Agent Conversation | New/existing session with agent — conversation, report, and chat panels |
...
- Allows users to highlight text within a graph (nodes, labels, or notes) and use an LLM to rewrite it via a contextual pop-up menu.
