...
For 1on1-prep-brief, see `1on1-prep-brief-Chris-Peterson.md` for a sample report. When `LLM_API_KEY` is configured, reports are generated by the LLM. When not configured, the sample data is used as mock fallback.
